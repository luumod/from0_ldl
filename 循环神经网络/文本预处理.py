...
if __name__ == '__main__':
    ...
    print(f'词元索引列表大小：{len(corpus)}')
    print(f'词汇表的大小：{len(vocab)}')
    print(f'词元频率字典（前10个）：{list(vocab.valid_token_freqs.items())}')

    for i in range(8):
        print(f'索引值为{i}的词元为：{vocab.get_token(i)!r}')

    string = 'CoCo, I love you the most, you must know that, right?'
    valid_string = tokenize(remove_non_alpha_and_lower(string))
    string_encode = vocab.encode(valid_string)

    print(f'文本：{string}')
    print(f'文本（清洗后）：{valid_string}')
    print(f'经词汇表转换的索引：{string_encode}')
    print(f'重新映射到的词元：{vocab.decode(string_encode)}')
